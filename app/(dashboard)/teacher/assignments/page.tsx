...
export default function TeacherAssignmentsPage() {
  ...
  const filtered = assignments.filter((a) => {
    ...
    if (search) {
      ...
      return a.title.toLowerCase().includes(q) || a.className.toLowerCase().includes(q);
    }
    return true;
  });
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Create, manage, and track assignments for all your classes.
          </p>
        </div>
                    ...
                    <TableRow key={a.id} className="hover:bg-muted/50">
                      ...
                      <TableCell className="text-right pr-6">
                        <div className="flex justify-end gap-1">
                          <Button size="icon-sm" variant="ghost" asChild title="View">
              ...
              <div className="space-y-3">
                ...
                <div className="grid gap-2 sm:grid-cols-2">
                  <div className="rounded-lg border p-3">
                    ...
                    <p className="text-sm font-medium">{assignments.find((a) => a.id === deleteAssignmentId)?.submissionCount || 0}</p>
                  </div>
                  <div className="rounded-lg border p-3">
                    <p className="text-xs text-muted-foreground">Graded</p>
                    <p className="text-sm font-medium">{assignments.find((a) => a.id === deleteAssignmentId)?.gradedCount || 0}</p>
                  </div>
                </div>
